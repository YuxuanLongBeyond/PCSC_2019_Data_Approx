...
public:
    ...
    /**
     * Calculate the MSE between the interpolated values from approximation by C++ code and MATLAB for code debugging
     ...
     */
    static double compute_mse(std::vector<double> Y_test, std::vector<double> Y_matlab);
     ...
     */
    static void debug(int use_debug_mode, int N_test, int poly_degree, double poly_lambda, const std::vector<double>& Y_test, const std::string& mat_file, const std::string& approx_method, Test test, const Fitter& approx, DataIO data_handler);
