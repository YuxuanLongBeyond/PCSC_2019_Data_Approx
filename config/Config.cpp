...
Config::Config( const string& filename, string delimiter,
                string comment )
        : m_Delimiter(std::move(delimiter)), m_Comment(std::move(comment))
{
...
bool Config::FileExist(const std::string& filename)
{
    bool exist= false;
    std::ifstream in( filename.c_str() );
    if( in )
...
void Config::ReadFile( const string& filename, string delimiter,
                       string comment )
{
